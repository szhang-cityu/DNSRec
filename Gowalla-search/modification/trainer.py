...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        self.start_epoch = 0
        self.cur_step = 0
        self.best_decision = 0 #Initialize the best_decision
        self.best_valid_score = -np.inf if self.valid_metric_bigger else np.inf
        self.best_valid_result = None
        self.train_loss_dict = dict()
        self.optimizer = torch.optim.Adam(
            params=[param for name, param in model.named_parameters()
                    if name != 'weights.deep_weights' and name != 'weights_layer.deep_weights_layer'],
            lr=self.learning_rate, weight_decay=self.weight_decay)
        # -----------------------set the optimizer for selection weights---------------------
        self.optimizer_darts = torch.optim.Adam(
            params=[param for name, param in model.named_parameters()
                    if name == 'weights.deep_weights'],
            lr=self.learning_rate, weight_decay=self.weight_decay) # Optimizer for pruning
        self.optimizer_darts_layer= torch.optim.Adam(
            params=[param for name, param in model.named_parameters()
                    if name == 'weights_layer.deep_weights_layer'],
            lr=self.learning_rate, weight_decay=self.weight_decay)  # Optimizer for layer search
    ...
    def _train_epoch(self, train_data, epoch_idx, stage, darts_frequency, loss_func=None, show_progress=False):
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            #---------------------Train regardless of stage---------------------
            scaler.step(self.optimizer)
            scaler.update()
            total_loss_out = total_loss

            ## -------------------------DARTS: train the selection weights----------------
            if stage == 1:
                if (batch_idx + 1) % darts_frequency == 0:
                    #--------------Calculate the loss again to regain the gradient---------------
                    interaction = interaction.to(self.device)
                    self.optimizer_darts.zero_grad()
                    sync_loss = 0
                    if not self.config["single_spec"]:
                        self.set_reduce_hook()
                        sync_loss = self.sync_grad_loss()
                    ...
                    # ------------------------------------layer_num search------------------------------
                    # Calculate the loss again to regain the gradient
                    self.optimizer_darts_layer.zero_grad()
                    sync_loss = 0
                    if not self.config["single_spec"]:
                        self.set_reduce_hook()
                        sync_loss = self.sync_grad_loss()
                    ...
                    self._check_nan(loss)
                    loss3 = loss + sync_loss
                    loss3.backward()
                    if self.clip_grad_norm:
                        clip_grad_norm_(self.model.parameters(), **self.clip_grad_norm)
                    self.optimizer_darts_layer.step()
    ...
    def _valid_epoch(self, valid_data, show_progress=False):
        #---------------------- validation with no decision-------------------
        valid_result = self.evaluate(
            valid_data, best_decision=0, best_decision_layer=0, load_best_model=False, show_progress=show_progress
        )
    ...
    def _save_checkpoint(self, epoch, verbose=True, **kwargs):
        ...
        state = {
            ...
            "Decision": self.best_decision,
            "Selection Weights": self.model.weights.deep_weights,
            "Selection Layer Weights": self.model.weights_layer.deep_weights_layer,
        }
    ...
    def fit(
        ...
        best_decision = 0 # Set 0 before getting best_decision
        best_decision_layer = 0  # Set 0 before getting best_decision
        for epoch_idx in range(self.start_epoch, self.epochs):
            # train
            training_start_time = time()
            train_loss = self._train_epoch(
                train_data, epoch_idx, stage, darts_frequency, show_progress=show_progress
            )
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                # ----------------------EARLY STOPPING FOR ARCHITECTURE SEARCH--------------------
                if epoch_idx == 9:
                    stop_flag = True
                #---------------------------------------------------------------------------------
                valid_end_time = time()
                ...
                # ---------------------Get the best decision in training stage------------------
                if update_flag == True:
                    best_decision = self.decide()
                    self.best_decision = best_decision
                    best_decision_layer = self.decide_layer()
                    self.best_decision_layer = best_decision_layer
                # ---------------------------------------------------------------------------------
                if verbose:
                    self.logger.info(valid_score_output)
                    self.logger.info(valid_result_output)
                    self.logger.info(self.best_decision)
                    self.logger.info(self.best_decision_layer)
                    self.logger.info(self.model.weights.deep_weights)
                    self.logger.info(self.model.weights_layer.deep_weights_layer)
                ...
                valid_step += 1
            print(f'Current best_decision for pruning= {best_decision}')#  Print decision for each epoch
            print(f'Current best_decision for number of layers: {best_decision_layer}')  # Print decision for each epoch
            print('Weights after this epoch: ', torch.softmax(self.model.weights.deep_weights, dim=0))
            print('Weights after this epoch: ', torch.softmax(self.model.weights_layer.deep_weights_layer, dim=0))

        self._add_hparam_to_tensorboard(self.best_valid_score)
        return self.best_valid_score, self.best_valid_result, self.best_decision, self.best_decision_layer
    ...
    def decide(self):
        ...
        if dec_f == 1:
            ...
            index = index.detach().cpu().numpy()
            decision = np.array([0] * index.shape[0])
            num = 1# choose one decision for each layer
            for i in index[:num]:#each index represents the choice in one block
                decision[i] = 1
        if dec_f == 2:
            # Hard Selection
        ...
        # ------------------------------Get the best decisions---------------------------
    def decide_layer(self):
        model = self.model
        dec_f = model.dec_f
        if dec_f == 1:
            # SoftMAX selection with drop n
            _, index = torch.softmax(model.weights_layer.deep_weights_layer, dim=0)[:, -1].sort(dim=0, descending=True)
            index = index.detach().cpu().numpy()
            decision_layer = np.array([0] * len(index))
            num = 1
            for i in index[:num]:
                decision_layer[i] = 1
    ...
    @torch.no_grad()
    def evaluate(
        self, eval_data, best_decision, best_decision_layer, load_best_model=True, model_file=None, show_progress=False
    ):
        r"""Evaluate the model based on the eval data.
        ...
        if load_best_model:
            ...
            self.model.load_state_dict(checkpoint["state_dict"])
            self.model.load_other_parameter(checkpoint.get("other_parameter"))
            # Reset the deep weights to control the block selection
            decided_weights = torch.zeros(self.option_size, self.candidate)
            k = 0
            for i in best_decision:
                decided_weights[k] = i
                k += 1
            self.model.weights.deep_weights = torch.nn.Parameter(decided_weights.to(self.device), requires_grad=False)
            # Reset the deep weights to control the hidden size selection
            decided_weights_layer = torch.zeros(self.n_layers, self.candidate)
            k = 0
            for i in best_decision_layer:
                decided_weights_layer[k] = i
                k += 1
            self.model.weights_layer.deep_weights_layer = torch.nn.Parameter(decided_weights_layer.to(self.device),
                                                                               requires_grad=False)
