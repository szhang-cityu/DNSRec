...
class GateNN(nn.Module):
    ...
    def forward(self, inputs):
        return self.gate(inputs) * 2 # Control the values of outputs，similar to batch normalization, in case of gradient drift

#Basic Multihead Attention
# class Basic_MultiHeadAttention(nn.Module):
#     """
#     Basic Multi-head Self-attention layers, a attention score dropout layer is introduced.
#
#     Args:
...
#         layer_norm_eps,
#     ):
#         super(Basic_MultiHeadAttention, self).__init__()
...
# class Sparse_MultiHeadAttention(nn.Module):
#     """
#     Sparse Multi-head Self-attention layers, a attention score dropout layer is introduced.
#
#     Args:
#         input_tensor (torch.Tensor): the input of the multi-head self-attention layer
#         attention_mask (torch.Tensor): the attention mask for input tensor
...
#         top_k = 30
#     ):
#         super(Sparse_MultiHeadAttention, self).__init__()
#         if hidden_size % n_heads != 0:
#             raise ValueError(
#                 "The hidden size (%d) is not a multiple of the number of attention "
#                 "heads (%d)" % (hidden_size, n_heads)
...
#         # Normalize the attention scores to probabilities.
#         attention_probs = self.softmax(attention_scores)
#         _, top_indices = torch.topk(attention_probs, self.top_k, dim=-1)
#         attention_probs = torch.zeros_like(attention_probs).scatter_(-1, top_indices, attention_probs.gather(-1, top_indices))
...
class SP_MultiHeadAttention(nn.Module):
    """
    ...
    def __init__(
        ...
        attn_dropout_prob,
        layer_norm_eps,
        mask_size, # prune the d size using auto-dim
    ):
        super(SP_MultiHeadAttention, self).__init__()
        if hidden_size % n_heads != 0:
            raise ValueError(
                "The hidden size (%d) is not a multiple of the number of attention "
                "heads (%d)" % (hidden_size, n_heads)
    ...
    #-------------------------Auto_dim of QKV-----------------------------------
    def mask_kqv(self, x, mask_size, weight):
        out = 0
        k = 0
        for i in mask_size:
            t = x.clone()
    ...
    #---------------------------Auto_dim of dense------------------------------
    def mask_dense(self, x, mask_size, weight):
        out = 0
        k = 0
        for i in mask_size:
            t = x.clone()
    ...
    def forward(self, input_tensor, attention_mask, weight):
        mixed_query_layer = self.query(input_tensor)
        mixed_key_layer = self.key(input_tensor)
        mixed_value_layer = self.value(input_tensor)

        #-------------------------------------Prune on the QKV------------------------
        mixed_query_layer = self.mask_kqv(mixed_query_layer, self.mask_size, weight)
        mixed_key_layer = self.mask_kqv(mixed_key_layer, self.mask_size, weight)
        mixed_value_layer = self.mask_kqv(mixed_value_layer, self.mask_size, weight)
...
#         context_layer = torch.matmul(attention_probs, value_layer)
        ...
        context_layer = context_layer.view(*new_context_layer_shape)
        hidden_states = self.dense(context_layer)
        #-----------------------------Prune the dense layer using auto-dim-------------------
        hidden_states = self.mask_dense(hidden_states, self.mask_size, weight)
...
class FeedForward(nn.Module):
    ...
    def __init__(
        self, hidden_size, inner_size, hidden_dropout_prob, hidden_act, layer_norm_eps, mask_ffn, mask_size
    ):
        super(FeedForward, self).__init__()
        self.dense_1 = nn.Linear(hidden_size, inner_size)
        self.gate_1 = GateNN(hidden_size, output_dim=inner_size)
        self.intermediate_act_fn = self.get_hidden_act(hidden_act)

        self.dense_2 = nn.Linear(inner_size, hidden_size)
        self.gate_2 = GateNN(hidden_size, output_dim=hidden_size)
        self.LayerNorm = nn.LayerNorm(hidden_size, eps=layer_norm_eps)
    ...
    def forward(self, input_tensor, input_tensor0, weight):
        hidden_states = self.dense_1(input_tensor)
        #--------------------------Prune the inner size-----------------------
        hidden_states = self.mask_dense(hidden_states, self.mask_ffn, weight)
        gate_1 = self.gate_1(input_tensor0)
        gate_1 = self.mask_dense(gate_1, self.mask_ffn, weight)
        hidden_states = self.intermediate_act_fn(hidden_states * gate_1)

        hidden_states = self.dense_2(hidden_states)
        # --------------------------Prune the hidden size-----------------------
        hidden_states = self.mask_dense(hidden_states, self.mask_size, weight)
        gate_2 = self.gate_2(input_tensor0)  # not used repeatedly
        gate_2 = self.mask_dense(gate_2, self.mask_size, weight)
...
class TransformerLayer(nn.Module):
    ...
    def __init__(
        ...
        self.attention = SP_MultiHeadAttention(
            n_heads, hidden_size, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps, mask_size,
        )
    ...
    def forward(self, hidden_states, input_tensor0, attention_mask, weight, FLOPs):
        # Scaled Product
        attention = self.attention(hidden_states, attention_mask, weight)
        # flop2, _ = profile(self.attention, inputs=(hidden_states, attention_mask,))
        feedforward = self.feed_forward(attention, input_tensor0, weight)

        block_output = feedforward
        # Calculate the FLOPs
        flop1, flop2, flop3 = FLOPs
        fmin = min([flop1, flop2, flop3])
        fmax = max([flop1, flop2, flop3])
        flop1 = flop1 / (fmax - fmin)
        flop2 = flop2 / (fmax - fmin)
        flop3 = flop3 / (fmax - fmin)
        flop = weight[0]*flop1 + weight[1]*flop2 + weight[2]*flop3
        return block_output, flop
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.hyper_flops = hyper_flops

    def forward(self, hidden_states, input_tensor0, attention_mask, weight, weight_layer, FLOPs, output_all_encoded_layers=True):
        """
        Args:
        ...
        """
        all_encoder_layers = []
        # Give different options for different Transformer block
        i = 0
        flops = 0
        hidden_states_final = 0
        for layer_module in self.layer:
            hidden_states, flop = layer_module(hidden_states, input_tensor0, attention_mask, weight, FLOPs)
            flops += flop * weight_layer[i] * (i+1)
            hidden_states_final += weight_layer[i] * hidden_states
            if output_all_encoded_layers:
                all_encoder_layers.append(hidden_states_final)
            i += 1
        if not output_all_encoded_layers:
            all_encoder_layers.append(hidden_states_final)
        # -------------------------flops check--------------------------
        _, index = weight_layer.sort(dim=0, descending=True)
        num_block = index[0]+1
        flops = self.hyper_flops * flops * num_block / 4  # 0.3 is the trade-off parameter
        # ---------------------------------------------------------------
        return all_encoder_layers, flops
