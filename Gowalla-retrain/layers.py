...
class GateNN(nn.Module):
    def __init__(self,
        ...
        gate_layers = [nn.Linear(input_dim, hidden_dim)]# Neural layer
        if batch_norm:
        ...
        gate_layers.append(nn.Linear(hidden_dim, output_dim))# Add another neural layer
...
class FeedForward(nn.Module):
    ...
    def forward(self, input_tensor, input_tensor0):
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor0, attention_mask):
        attention_output = self.multi_head_attention(hidden_states, attention_mask)
        feedforward_output = self.feed_forward(attention_output, input_tensor0)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor0, attention_mask, output_all_encoded_layers=True):
        """
        Args:
        ...
        for layer_module in self.layer:
            hidden_states = layer_module(hidden_states, input_tensor0, attention_mask)
